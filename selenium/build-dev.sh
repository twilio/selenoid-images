...
if [ -z "$1" -o -z "$2" ]; then
    echo 'Usage: build-dev.sh {firefox/apt/{stable|beta|dev}|firefox/local/{stable|beta|dev}|chrome/apt/{stable|beta|dev}|chrome/local/{stable|beta|dev}|opera/presto|opera/blink/local|opera/blink/apt|yandex/local|yandex/apt} <browser_version> [<cleanup={true|false}>] [<requires_java={true|false}>] [<tag_version>] '
    exit 1
fi
set -x
